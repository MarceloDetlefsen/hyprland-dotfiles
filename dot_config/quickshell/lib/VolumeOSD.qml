...
PanelWindow {
    ...
    Lib.CommandPoll {
        ...
        interval: 500
        running: root.osdVisible
        command: ["bash", "-lc", "pactl get-sink-mute @DEFAULT_SINK@ 2>/dev/null | awk '{print $2}'"]
        parse: function(o) {
            var s = String(o).trim()
            return s === "yes" || s === "true"
        }
        onUpdated: root.muted = Boolean(value)
    }
    ...
    Timer {
        ...
        interval: 650
        repeat: false
        onTriggered: root.muteFlash = false
    }
    ...
    Item {
        ...
        SequentialAnimation on rotation {
            id: muteShakeAnim
            running: false
            NumberAnimation { to: -1.8; duration: 45; easing.type: Easing.OutQuad }
            NumberAnimation { to:  1.8; duration: 45; easing.type: Easing.OutQuad }
            NumberAnimation { to: -0.8; duration: 45; easing.type: Easing.OutQuad }
            NumberAnimation { to:  0; duration: 65; easing.type: Easing.OutQuad }
        }
            ...
            Rectangle {
                ...
                Rectangle {
                    ...
                    color:        root.muted
                                  ? Qt.rgba(Qt.color(root.theme.accentRed).r,
                                            Qt.color(root.theme.accentRed).g,
                                            Qt.color(root.theme.accentRed).b,
                                            root.muteFlash ? 0.18 : 0.08)
                                  : "transparent"
                    opacity:      root.muteFlash || root.muted ? 1.0 : 0.0
                    Behavior on opacity {
                    ...
                    ColumnLayout {
                        ...
                        Item {
                            ...
                            Text {
                                id: volIcon
                                anchors.centerIn: parent
                                visible:          !root.isHeadphones && !root.muted
                            ...
                            Rectangle {
                                ...
                                height: 28
                                radius: 14
                                visible: root.muted || root.muteFlash
                                opacity: visible ? 1.0 : 0.0
                                scale: root.muteFlash ? 1.08 : 1.0
                                color: Qt.rgba(
                                    Qt.color(root.theme.accentRed).r,
                                    Qt.color(root.theme.accentRed).g,
                                    Qt.color(root.theme.accentRed).b,
                                    0.20
                                )
                                ...
                                Image {
                                    ...
                                    width: 20
                                    height: 20
                                    source: Qt.resolvedUrl("mute.svg")
                                    visible: false
                                    smooth: true
                                    mipmap: true
                                }
    ...
    Connections {
        ...
        function onMutedChanged()  {
            ...
            muteScaleAnim.restart()
            muteShakeAnim.restart()
            if (root.muted) muteFlashTimer.restart()
        }
    }
}
}
