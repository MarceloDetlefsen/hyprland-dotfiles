...
Lib.Card {
  ...
  Lib.CommandPoll {
    ...
    command: Lib.Shell.sh("~/.config/hypr/scripts/bluetooth-control.sh state")
    parse: function(o) {
      var s = String(o).trim()
      return s === "true" || s === "1"
    }
  ...
  function toggleBt() { 
      ...
      optTimer.restart();
      Lib.Shell.det("~/.config/hypr/scripts/bluetooth-control.sh toggle")
  }
  // --------------------------------------------------------------------------------------------------------------
  // Volume
  Lib.CommandPoll {
  ...
  Lib.CommandPoll {
    id: mutePoll;
    running: root.active && root.visible; interval: 900
  ...
  Lib.CommandPoll {
    ...
    command: Lib.Shell.sh("makoctl mode 2>/dev/null || true")
    parse: function(o) { return String(o).indexOf("do-not-disturb") !== -1 }
    onUpdated: root.dnd = value
  }
  function toggleDnd() { var next = !root.dnd;
  root.dnd = next;
  Lib.Shell.det("makoctl mode " + (next ? "-a do-not-disturb" : "-r do-not-disturb"));
  Lib.Shell.det("makoctl reload >/dev/null 2>&1 || true");
  ...
  RowLayout {
    ...
    ColumnLayout {
      ...
      FitButton {
        ...
        onRightClicked: {
            root.closeRequested()
            Lib.Shell.det("~/.config/hypr/scripts/bluetooth-menu.sh")
        }
      }
    ...
    RowLayout {
        ...
        rotation: -90
        theme: root.theme
        icon: { if (mutePoll.value || value === 0) return "mute.svg";
        if (volPoll.value.isHeadphones) return "vol_headphones.svg"; return (value > 50) ? "vol_50p.svg" : "vol_50m.svg" }
        from: 0;
        to: 100; value: 0
        accentColor: mutePoll.value ? (root.theme && root.theme.accentRed ? root.theme.accentRed : Theme.accentRed) : ((root.theme && !root.theme.isDarkMode) ?
        root.theme.accentSlider : "#83C092")
