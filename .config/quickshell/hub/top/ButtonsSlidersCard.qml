...
Lib.TopCard {
  ...
  Lib.CommandPoll {
    ...
    command: sh("~/.config/hypr/scripts/bluetooth-control.sh state")
    parse: function(o) {
      var s = String(o).trim()
      return s === "true" || s === "1"
    }
  ...
  function toggleBt() { 
      ...
      optTimer.restart();
      det("~/.config/hypr/scripts/bluetooth-control.sh toggle")
  }
  ...
  Lib.CommandPoll {
    ...
    running: root.active && root.visible
    interval: 900
    command: sh("pactl get-sink-mute @DEFAULT_SINK@ 2>/dev/null | awk '{print $2}'")
    parse: function(o) {
      var s = String(o).trim()
      return s === "yes" || s === "true"
    }
...
// --- DND ---
  ...
  function toggleDnd() {
    var next = !root.dnd
    root.dnd = next
    det("makoctl mode " + (next ? "-a do-not-disturb" : "-r do-not-disturb"))
    det("makoctl reload >/dev/null 2>&1 || true")
    det("makoctl dismiss --all >/dev/null 2>&1 || true")
  }
  ...
  ColumnLayout {
    ...
    RowLayout {
      ...
      Lib.ExpressiveButton {
        ...
        onRightClicked: {
            root.closeRequested()
            det("~/.config/hypr/scripts/bluetooth-menu.sh")
        }
      }
  ...
  ColumnLayout {
    ...
    Layout.fillWidth: true
      ...
      Lib.ExpressiveSlider {
        ...
        icon: {
            if (mutePoll.value || value === 0) return "mute.svg"
        ...
        value: 0
        Layout.fillWidth: true
        accentColor: mutePoll.value
            ? (root.theme && root.theme.accentRed ? root.theme.accentRed : "#d96b6b")
            : ((root.theme && root.theme.isDarkMode !== undefined && !root.theme.isDarkMode)
                ? root.theme.accentSlider
                : "#83C092")
