...
PanelWindow {
   ...
   // -----------------------------------------------
    ...
    // Performance / connectivity / audio / brightness
    Lib.CommandPoll {
        id: cpuPoll
        interval: 2500
        command: Lib.Shell.sh("bash ~/.config/quickshell/bin/qs_cpu_usage.sh")
        parse: function(o) { var n = parseInt(String(o ?? "").trim()); return isFinite(n) ? n : 0 }
    }

    Lib.CommandPoll {
        id: ramPoll
        interval: 4000
        command: Lib.Shell.sh("bash ~/.config/quickshell/bin/qs_ram_usage.sh")
        parse: function(o) { var n = parseInt(String(o ?? "").trim()); return isFinite(n) ? n : 0 }
    }
    ...
    Lib.CommandPoll {
        id: mutePoll
        interval: 900
        command: Lib.Shell.sh("pactl get-sink-mute @DEFAULT_SINK@ 2>/dev/null | awk '{print $2}'")
        parse: function(o) { var s = String(o ?? "").trim(); return s === "yes" || s === "true" }
    }
    ...
    Lib.CommandPoll {
        id: wifiPercent
        interval: 5000
        command: Lib.Shell.sh("bash ~/.config/quickshell/bin/qs_wifi_percent.sh")
        parse: function(o) { var n = parseInt(String(o ?? "").trim()); return isFinite(n) ? n : 0 }
    }
    ...
    function cpuIcon() { return "" }
    function ramIcon() { return "" }
...
// ============================================================================================================================================================================================================================================================================================================================
            ...
            // --------------------------------------------
                ...
                // CENTER: current window / playing media
                Item {
                    ...
                    property bool hasMedia: {
                        ...
                        const title = String(mediaPlayer.trackTitle || "").trim()
                        const artist = String(mediaPlayer.trackArtist || "").trim()
                        const label = title || artist
                        return label.length > 0 && !/^spotify free$/i.test(label)
                    }
                    property string mediaTitle: hasMedia ? String(mediaPlayer.trackTitle || "") : ""
                    property string mediaArtist: hasMedia ? String(mediaPlayer.trackArtist || "") : ""
                    property string mediaLabel: mediaTitle || mediaArtist || ""
                    ...
                    Item {
                        ...
                        Row {
                            ...
                            Text {
                                id: mediaText
                                width: Math.min(220, Math.max(90, parent.parent.parent.width * 0.34))
                                text: String(parent.parent.parent.mediaLabel || "")
                                font.family: theme.textFont
                                font.weight: 700
                                font.pixelSize: 12
                                color: pal.textPrimary
                    ...
                    Text {
                        id: windowTitle
                        anchors.left: musicBlock.right
                        anchors.leftMargin: musicBlock.visible ? 12 : 0
                        anchors.right: parent.right
                        ...
                        color: pal.textPrimary
                        verticalAlignment: Text.AlignVCenter
                        horizontalAlignment: parent.parent.hasMedia ? Text.AlignHCenter : Text.AlignLeft
                        elide: Text.ElideRight
                        Behavior on width {
                ...
                RowLayout {
                    ...
                    TaskbarItem {
                        ...
                        icon: "󰋊"
                        text: String(storagePoll.value ?? 0) + "%"
                        bgColor: pal.bg
                        iconColor: (Number(storagePoll.value) >= 90)
                            ? (taskbar.isDarkMode ? "#e67e80" : "#a55524")
                            : pal.textPrimary
                        textColor: (Number(storagePoll.value) >= 90)
                            ? (taskbar.isDarkMode ? "#e67e80" : "#a55524")
                            : pal.textPrimary
                    }
                    ...
                    TaskbarItem {
                        ...
                        icon: cpuIcon()
                        text: String(cpuPoll.value ?? 0) + "%"
                        bgColor: pal.bg
                        iconColor: (Number(cpuPoll.value) >= 80) ? (taskbar.isDarkMode ? "#e67e80" : "#a55524") : pal.textPrimary
                        textColor: (Number(cpuPoll.value) >= 80) ? (taskbar.isDarkMode ? "#e67e80" : "#a55524") : pal.textPrimary
                    }

                    TaskbarItem {
                        Layout.preferredWidth: 62
                        icon: ramIcon()
                        text: String(ramPoll.value ?? 0) + "%"
                        bgColor: pal.bg
                        iconColor: (Number(ramPoll.value) >= 80) ? (taskbar.isDarkMode ? "#e69875" : "#a55524") : pal.textPrimary
                        textColor: (Number(ramPoll.value) >= 80) ? (taskbar.isDarkMode ? "#e69875" : "#a55524") : pal.textPrimary
                    }
                    ...
                    TaskbarItem {
                        Layout.preferredWidth: 66
                        icon: brightnessIcon(Number(briPoll.value) || 0)
                        text: String(briPoll.value ?? 0) + "%"
                        bgColor: pal.bg
                    ...
                    TaskbarItem {
                        Layout.preferredWidth: 66
                        icon: volumeIcon(Number(volPoll.value) || 0, Boolean(mutePoll.value))
                        text: String(volPoll.value ?? 0) + "%"
                        bgColor: pal.bg
                        iconColor: Boolean(mutePoll.value) ? (taskbar.isDarkMode ? "#e67e80" : "#a55524") : pal.textPrimary
                        textColor: Boolean(mutePoll.value) ? (taskbar.isDarkMode ? "#e67e80" : "#a55524") : pal.textPrimary
                    }
